...
- 무한한 바다를 배경으로 한 뱀파이어 서바이벌류 2D 로그라이크 게임입니다. 어부가 작살과 총을 사용하여 바다 괴물들과 싸우는 게임입니다. (뱀서라이크 Tutorial 참고, Gemini CLI 사용)
- 몬스터 논의 & 제작, 무한 맵 만들기(타일기반), 몬스터 풀링(메모리 관리), 스폰율(빈도 수) 제작
...
- 몬스터 넉백 및 미끄러짐과 관련된 물리 오류 수정.

## 🎮 핵심 플레이 화면
![게임 플레이 미리보기](Playimages/PlayVideo.gif)

## 아쉬운점
- **날씨 & 시간 구현(흐린맵, 어두운 맵):** 날씨(비 배경, 폭풍)구현과 시간에 따른 다른 몬스터를 구현했으면 더 좋았을듯 
- **몬스터 물리 문제 구현:** 몬스터가 더 다양한 움직임을 표현했어야함
- **보스 몬스터의 구현:** 게임을 마무리하는 보스몬스터의 구현을 못했다는 것이 아쉬움
- **레벨 분화:** 단계를 세분화하는 레벨 디자인 경험이 부족함
